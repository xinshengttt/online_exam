...
const updateStudentCourse = async () => {
  ...
  try {
    const res = await myRequest.put<any, ApiResult>(
      "/course/updateStudentByCourse",
      form
    );
...
const handleDelete = (row) => {
  ElMessageBox.confirm("此操作将永久删除数据, 是否继续?", "提示", {
    ...
    .then(async () => {
      const res = await request.delete<any, ApiResult>(
        `/user/deleteStudentByCourse/${row.ucId}`
      );
